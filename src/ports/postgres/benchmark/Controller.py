    ...
    def __init__(self, env, controller):
        ...
        parser = ArgumentParser(
            description="MADlib performance testing (" +
                str(env.madlib_version) + ", benchmark platform: " + env.platform +
                ")",
...
class Controller:
    ...
    def runSQL(self, sql, psqlArgs = None, stdinCmd=None):
        """
        @brief Run SQL commands with psql and return output
        @param: sql, sql statement
        @param: psqlArgs
        @param: utility, whether this sql should run as utility mode for greenplum
        @param: stdinCmd, stdin command line if the sql should copy data from stdin
        """
        env = self.env
        # See
        ...
        if stdinCmd:
            ...
            psqlProcess = subprocess.Popen(psqlCmd, env = environ, shell=True, stdin=stdinProcess.stdout, stdout=subprocess.PIPE, stderr = None)
            ...
            (stdoutdata, _) = psqlProcess.communicate(sql)


        if psqlProcess.returncode != 0:
            raise PSQLError(psqlProcess.returncode)
