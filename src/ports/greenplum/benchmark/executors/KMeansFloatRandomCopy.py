...
class ModuleEnvironment(Environment):
    def __init__(self, env):
        ...
        parser = ArgumentParser(
            description="KMeans on Postgres", prog = env.prog +
                ' [...] ' + env.benchmark)
...
class Executor:
    """
    Postgres k-means test with random data
    """
    ...
    def load(self):
        ...
        with timer:
            cmdCreateData = '''{theGenerator} --dimensions {dimensions} --rows      {rows} --centroids {centroids} --stddev 0.01 -s 10 --table'''\
            ...
            cmdCreateModel = '''{theGenerator} --dimensions {dimensions} --centroids {centroids} --centroid'''\
                .format(theGenerator = postgresGenerator, **env.properties_and_attributes())

            result = result + self.controller.runSQL(
                '''
                CREATE TABLE pg_temp.model (
                    centroid FLOAT[]
                );
                COPY pg_temp.model From stdin;
